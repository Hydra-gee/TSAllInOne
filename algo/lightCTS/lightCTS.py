...
from algo.lightCTS.transformer_model import Transformer
...
class Model(nn.Module):
    def __init__(self, expConfig,modelConfig,dropout=0.1, supports=None, in_dim=1, out_dim=12, hid_dim=32, layers=8, cnn_layers=4, group=4):
        super(Model, self).__init__()
        out_dim = expConfig['pred_len']
        ...
        # mask = mask0 + mask1
        # self.mask = mask == 0
        self.mask = torch.zeros(expConfig['dim'], dtype=torch.bool).to(expConfig['device'])
        # self.mask = torch.zeros(7).to(expConfig['device'])
